...
from ocr_evaluation.utils import resize_image, clean_ground_truth
...
def evaluate_easyocr(dataset_path, annotation_file, num_images=None):
    """Оценка EasyOCR на Dataset1 с сохранением результатов в CSV."""
    config = DATASETS['Dataset1']
    lang = config['lang']
    try:
    ...
    csv_output_path = os.path.join(PROCESSED_DATA_DIR, 'results_dataset1.csv')
    ...
    json_output_path = os.path.join(dataset_path, 'ocr_results_dataset1.json')
