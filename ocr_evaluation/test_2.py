...
"""
Оценка EasyOCR на датасете Dataset2 с сохранением результатов в CSV
"""
...
from ocr_evaluation.utils import clean_ground_truth
...
def evaluate_easyocr(dataset_path, annotation_file, num_images=None):
    """Оценка EasyOCR на Dataset2 с сохранением результатов в CSV."""
    config = DATASETS['Dataset2']
    lang = config['lang']
    try:
    ...
    logging.info(f'Dataset2 - Средний CER: {avg_cer:.4f}, Средний WER: {avg_wer:.4f}')

    # Сохранение результатов в CSV
    results_df = pd.DataFrame(results)
    csv_output_path = os.path.join(PROCESSED_DATA_DIR, 'results_dataset2.csv')
    ...
    json_output_path = os.path.join(dataset_path, 'ocr_results_dataset2.json')
