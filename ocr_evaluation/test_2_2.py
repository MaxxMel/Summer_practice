...
def evaluate_easyocr(csv_path):
    """Оценка EasyOCR на dataset2_2 с сохранением результатов в CSV."""
    config = DATASETS['dataset2_2']
    lang = config['lang']
    ...
    debug_log_path = os.path.join(DATA_DIR, 'debug_dataset2_2.txt')
    with open(debug_log_path, 'w') as f:
        f.write('Debug log for dataset2_2\n')

    for idx, row in tqdm(df.iterrows(), total=len(df), desc='Обработка dataset2_2'):
        ...
        try:
            ...
            bbox = parse_bbox(bbox_str, img_width=img_width, img_height=img_height, dataset_name='dataset2_2')
    ...
    logging.info(f'dataset2_2 - Средний CER: {avg_cer:.4f}, Средний WER: {avg_wer:.4f}')

    # Сохранение результатов в CSV
    results_df = pd.DataFrame(results)
    csv_output_path = os.path.join(PROCESSED_DATA_DIR, 'results_dataset2_2.csv')
    ...
    json_output_path = os.path.join(DATA_DIR, 'ocr_results_dataset2_2.json')
