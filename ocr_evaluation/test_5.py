...
def evaluate_easyocr(csv_path):
    ...
    if 'Dataset5' not in DATASETS:
        logging.error("Конфигурация для 'dataset5' отсутствует в DATASETS. Проверьте config.py.")
        raise KeyError("Конфигурация для 'dataset5' не найдена в DATASETS")

    config = DATASETS['Dataset5']
    lang = config['lang']
    ...
    debug_log_path = os.path.join(DATA_DIR, 'debug_dataset5.txt')
    with open(debug_log_path, 'w') as f:
        f.write('Debug log for dataset5\n')

    for idx, row in tqdm(df.iterrows(), total=len(df), desc='Обработка dataset5'):
        ...
        try:
            ...
            bbox = parse_bbox(bbox_str, dataset_name='Dataset5')
    ...
    csv_output_path = os.path.join(PROCESSED_DATA_DIR, 'results_dataset5.csv')
    ...
    json_output_path = os.path.join(DATA_DIR, 'ocr_results_dataset5.json')
