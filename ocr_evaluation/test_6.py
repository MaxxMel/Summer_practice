...
from ocr_evaluation.config import DATA_DIR, PROCESSED_DATA_DIR, DATASETS
from ocr_evaluation.utils import resize_image, parse_bbox, clean_ground_truth

# Настройка логирования
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

def evaluate_easyocr(csv_path):
    """Оценка EasyOCR на Dataset6 с сохранением результатов в CSV."""
    if 'Dataset6' not in DATASETS:
        logging.error("Конфигурация для 'Dataset6' отсутствует в DATASETS. Проверьте config.py.")
        raise KeyError("Конфигурация для 'Dataset6' не найдена в DATASETS")

    config = DATASETS['Dataset6']
    lang = config['lang']
    ...
    debug_log_path = os.path.join(DATA_DIR, 'debug_dataset6.txt')
    with open(debug_log_path, 'w') as f:
        f.write('Debug log for Dataset6\n')

    for idx, row in tqdm(df.iterrows(), total=len(df), desc='Обработка Dataset6'):
        ...
        try:
            ...
            bbox = parse_bbox(bbox_str, dataset_name='Dataset6')
    ...
    logging.info(f'Dataset6 - Средний CER: {avg_cer:.4f}, Средний WER: {avg_wer:.4f}')

    # Сохранение результатов в CSV
    results_df = pd.DataFrame(results)
    csv_output_path = os.path.join(PROCESSED_DATA_DIR, 'results_dataset6.csv')
    ...
    json_output_path = os.path.join(DATA_DIR, 'ocr_results_dataset6.json')
...
def main():
    """Оценка EasyOCR на Dataset6."""
    csv_path = os.path.join(PROCESSED_DATA_DIR, 'dataset6_test.csv')
