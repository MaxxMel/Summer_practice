...
"""
Оценка EasyOCR на датасете Dataset6 с сохранением результатов в CSV
"""
...
def evaluate_easyocr(csv_path):
    """Оценка EasyOCR на ocr_machine_readable_zone_mrz_detection с сохранением результатов в CSV."""
    if 'ocr_machine_readable_zone_mrz_detection' not in DATASETS:
        logging.error("Конфигурация для 'ocr_machine_readable_zone_mrz_detection' отсутствует в DATASETS. Проверьте config.py.")
        raise KeyError("Конфигурация для 'ocr_machine_readable_zone_mrz_detection' не найдена в DATASETS")

    config = DATASETS['ocr_machine_readable_zone_mrz_detection']
    lang = config['lang']
    ...
    debug_log_path = os.path.join(DATA_DIR, 'debug_ocr_machine_readable_zone_mrz_detection.txt')
    with open(debug_log_path, 'w') as f:
        f.write('Debug log for ocr_machine_readable_zone_mrz_detection\n')

    for idx, row in tqdm(df.iterrows(), total=len(df), desc='Обработка ocr_machine_readable_zone_mrz_detection'):
        ...
        try:
            ...
            bbox = parse_bbox(bbox_str, dataset_name='ocr_machine_readable_zone_mrz_detection')
    ...
    logging.info(f'ocr_machine_readable_zone_mrz_detection - Средний CER: {avg_cer:.4f}, Средний WER: {avg_wer:.4f}')

    # Сохранение результатов в CSV
    results_df = pd.DataFrame(results)
    csv_output_path = os.path.join(PROCESSED_DATA_DIR, 'results_ocr_machine_readable_zone_mrz_detection.csv')
    ...
    json_output_path = os.path.join(DATA_DIR, 'ocr_results_ocr_machine_readable_zone_mrz_detection.json')
...
def main():
    """Оценка EasyOCR на ocr_machine_readable_zone_mrz_detection."""
    csv_path = os.path.join(PROCESSED_DATA_DIR, 'ocr_machine_readable_zone_mrz_detection_test.csv')
