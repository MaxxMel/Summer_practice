...
"""
Оценка EasyOCR на датасете dataset2_2 с сохранением результатов в CSV
"""
...
def evaluate_easyocr(csv_path):
    """Оценка EasyOCR на russian_language_visual_text_recognition с сохранением результатов в CSV."""
    config = DATASETS['russian_language_visual_text_recognition']
    lang = config['lang']
    ...
    debug_log_path = os.path.join(DATA_DIR, 'debug_russian_language_visual_text_recognition.txt')
    with open(debug_log_path, 'w') as f:
        f.write('Debug log for russian_language_visual_text_recognition\n')

    for idx, row in tqdm(df.iterrows(), total=len(df), desc='Обработка russian_language_visual_text_recognition'):
        ...
        try:
            ...
            bbox = parse_bbox(bbox_str, img_width=img_width, img_height=img_height, dataset_name='russian_language_visual_text_recognition')
    ...
    logging.info(f'russian_language_visual_text_recognition - Средний CER: {avg_cer:.4f}, Средний WER: {avg_wer:.4f}')

    # Сохранение результатов в CSV
    results_df = pd.DataFrame(results)
    csv_output_path = os.path.join(PROCESSED_DATA_DIR, 'results_russian_language_visual_text_recognition.csv')
    ...
    json_output_path = os.path.join(DATA_DIR, 'ocr_results_russian_language_visual_text_recognition.json')
...
def main():
    """Оценка EasyOCR на russian_language_visual_text_recognition."""
    csv_path = os.path.join(PROCESSED_DATA_DIR, 'russian_language_visual_text_recognition_test.csv')
