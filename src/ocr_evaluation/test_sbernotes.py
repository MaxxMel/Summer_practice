...
def evaluate_easyocr(csv_path, chunk_size=50, save_interval=10):
    ...
    debug_log_path = os.path.join(DATA_DIR, 'debug_sbernotes.txt')
    with open(debug_log_path, 'w') as f:
        f.write('Debug log for sbernotes\n')
    ...
    for chunk_start in range(0, total_images, chunk_size):
        ...
        for img_idx, (img_path, group) in enumerate(tqdm(chunk_groups, desc=f'Обработка sbernotes (чанк {chunk_start//chunk_size + 1})')):
            try:
                ...
                for _, row in group.iterrows():
                    ground_truth = clean_ground_truth(row['label'])
                    bbox_str = row['bbox']
                    bbox = parse_bbox(bbox_str, dataset_name='sbernotes')
                ...
                if (img_idx + 1) % save_interval == 0:
                    np.savez(os.path.join(DATA_DIR, f'intermediate_results_sbernotes_chunk_{chunk_start//chunk_size + 1}.npz'), wers=wer_scores, cers=cer_scores)
    ...
    csv_output_path = os.path.join(PROCESSED_DATA_DIR, 'results_sbernotes.csv')
    ...
    json_output_path = os.path.join(DATA_DIR, 'ocr_results_sbernotes.json')
    with open(json_output_path, 'w') as f:
        json.dump(json_results, f, indent=4)
    logging.info(f'Результаты сохранены в {json_output_path}')

    # Сохранение финальных результатов в NPZ
    np.savez(os.path.join(DATA_DIR, 'final_results_sbernotes.npz'), wers=wer_scores, cers=cer_scores, avg_wer=avg_wer, avg_cer=avg_cer)
    logging.info(f'Финальные результаты сохранены в {os.path.join(DATA_DIR, "final_results_sbernotes.npz")}')

    logging.info(f'Отладочная информация сохранена в {debug_log_path}')
    return avg_cer, avg_wer

def main():
    """Оценка EasyOCR на sbernotes."""
    csv_path = os.path.join(PROCESSED_DATA_DIR, 'sbernotes_test.csv')
