...
def evaluate_easyocr(csv_path):
    try:
        reader = easyocr.Reader(['en'], gpu=True)  # Только английский для MRZ-текста
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc='Обработка изображений'):
        ...
        try:
            ...
            cropped_image = resize_image(cropped_image, max_size=1024)  # Масштабирование для избежания проблем с памятью
    ...
    avg_wer6 = np.mean(wers) if wers else 0.0
    avg_cer6 = np.mean(cers) if cers else 0.0
    logging.info(f'Average WER: {avg_wer6:.4f}, Average CER: {avg_cer6:.4f}')

    return avg_wer6, avg_cer6

def main():
    csv_path = os.path.join(PROCESSED_DATA_DIR, 'dataset6_test.csv')
    avg_wer6, avg_cer6 = evaluate_easyocr(csv_path)
    logging.info(f'Final Results: Average WER = {avg_wer6:.4f}, Average CER = {avg_cer6:.4f}')
